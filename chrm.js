...
function registerPlotLocus(buttonId, lmsChart, primChart) {
  $(buttonId).on('click', function(evt) {
    ...
    // no need to scale (by rRad/gRad/bRad), since the primary SPDs are calculated (in |plotLMSPrims|) as ones that generated one unit of each primary.
    if (val == 'usePreset3') {
      rRad = 1;
      gRad = 1;
      bRad = 1;
...
function plotLMSPrims() {
  // find lights that exclusively stimulate the cones at unity. why stimulate cones at *unity*? by definition reference white in LMS is defined as one that's generated by one unit of each primary (cone sensitivities are used as the CMFs, so one unit of L cone response is read as one unit of L primary). so what we are calculating here is directly the SPDs of the primaries that generate one unit of each primary and so they won't have to be scaled later.
